...
@section('breadcrumb')
  ...
  <ol class="breadcrumb mb-4">
    <li class="breadcrumb-item"><a href="{{ route('hydrant.index') }}" class="text-decoration-none text-secondary">List</a></li>
      ...
      <div class="card mb-4">
        <div class="card-header">
          <div class="d-flex justify-content-between">
            <a href="{{ route('hydrant.index') }}" class="text-decoration-none text-dark"><i class="fa-solid fa-arrow-left"></i> Back</a>
          </div>
        </div>
        <div class="card-body p-1 bg-light">
          <div class="form-input">
            <form action="{{ route('hydrant.store') }}" method="POST">
              @csrf
              ...
              <div class="mb-3">
                  ...
                  <input type="text" class="form-control @error('number_hydrant') is-invalid @enderror" 
                        ...
                        value="{{ old('number_hydrant') }}" required>
                  ...
                  <div class="mb-2">
                    <small class="text-muted">Last created : 
                      <strong>{{ $lastHydrant ? $lastHydrant->number_hydrant : 'Belum ada data' }}</strong>
                    </small>
                  </div>
              </div>
                  ...
                  <div>
                      <div class="form-check form-check-inline">
                          <input class="form-check-input" type="radio" name="is_active" 
                                id="is_active_true" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
                          <label class="form-check-label" for="is_active_true">Aktif</label>
                      </div>
                      <div class="form-check form-check-inline">
                          <input class="form-check-input" type="radio" name="is_active" 
                                id="is_active_false" value="0" {{ !old('is_active', true) ? 'checked' : '' }}>
              ...
              <div class="mb-3">
                ...
                <select name="building_id" id="building_id" class="form-select @error('building_id')
                  is-invalid
                @enderror" required>
              ...
              <div class="mb-3">
                ...
                <select name="floor_id" id="floor_id" class="form-select @error('floor_id')
                  is-invalid
                @enderror" required>
              ...
              <div class="mb-3">
                ...
                <select name="brand_id" id="brand_id" class="form-select @error('brand_id')
                  is-invalid
                @enderror" required>
              ...
              <div class="mb-3">
                ...
                <select name="hydrant_type_id" id="hydrant_type_id" class="form-select @error('hydrant_type_id')
                  is-invalid
                @enderror" required>
              <option value="">Pilih Tipe</option>
              @foreach ($hydrantTypes as $hydrantType)
                <option value="{{ $hydrantType->id }}" {{ old('hydrant_type_id') == $hydrantType->id ? 'selected' : '' }}>{{ $hydrantType->name }}</option>
              @endforeach
              </select>
              ...
              <div class="mb-3">
                ...
                <select name="extinguisher_condition_id" id="extinguisher_condition_id" class="form-select @error('extinguisher_condition_id')
                  is-invalid
                @enderror" required>
              <option value="">Pilih Kondisi</option>
              @foreach ($extinguisherConditions as $extinguisherCondition)
                <option value="{{ $extinguisherCondition->id }}" {{ old('extinguisher_condition_id') == $extinguisherCondition->id ? 'selected' : '' }}>{{ $extinguisherCondition->name }}</option>
              @endforeach
              </select>
